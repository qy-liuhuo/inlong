 ...
 * copy from {@link com.ververica.cdc.connectors.mysql.source.reader.MySqlSourceReader}
 * */
public class MySqlSourceReader<T>
        extends
            SingleThreadMultiplexSourceReaderBase<SourceRecords, T, MySqlSplit, MySqlSplitState> {
    ...
    public List<MySqlSplit> snapshotState(long checkpointId) {
        if (metricSchema instanceof RowDataDebeziumDeserializeSchema) {
            ((RowDataDebeziumDeserializeSchema) metricSchema).updateCurrentCheckpointId(checkpointId);
        }
    ...
    public void notifyCheckpointComplete(long checkpointId) throws Exception {
        ...
        if (metricSchema instanceof RowDataDebeziumDeserializeSchema) {
            RowDataDebeziumDeserializeSchema schema = (RowDataDebeziumDeserializeSchema) metricSchema;
            schema.flushAudit();
